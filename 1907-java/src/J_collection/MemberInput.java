...
public class MemberInput extends JInternalFrame {
	...
	//메인프레임에서 얘를가지고 써야함 객체를생성할때 이 유형의 생성자를통해 만들어지도록 메인프레임에
	public MemberInput(List<MemberVo> list) {//list타입을 전달받을것임 //오버라이딩//리스트 타입의 list라는 변수
		this();
		this.list=list;
	}
	...
	public void input(){
		...
		status.setText("회원정보가  입력되었습니다"+ "("+list.size()+"건)");
