...
public class ProductInput extends JInternalFrame {
	...
	public void input() {
		...
		if(pName.getText().equals("")||ea.getText().equals("")||pCode.getText().equals("")) {
		...
		Date n = sdf.parse(nal.getText());//심플데이터 포맷의 파스 // 날자형태의 문자열을 날짜로 바꿔라
		
		ProductVo vo = new ProductVo(serial,pC,pN,e,n);//
		pi.add(vo);//저장하기위해 해쉬코드를 호출한다 - 동일한 객체가 들어온다하면 추가로 입력되지않는다 - 다음으로 이퀄즈를 호출해서 정말 같은것인지 판별한다
		
		MemberMain.iSerial++;
		status.setText("CODE:'"+pC+"' 제품이 등록되었습니다");
		pName.setText("");
		ea.setText("");
		pCode.requestFocus();
		pCode.selectAll();
			...
			status.setText("날짜를 yyyy-MM-dd 형식으로 입력해주세요");
			nal.requestFocus();
			nal.selectAll();
		} catch (NumberFormatException e2) {
			status.setText("수량은 숫자만 입력해주세요");
			ea.requestFocus();
			ea.selectAll();
		}
	}
